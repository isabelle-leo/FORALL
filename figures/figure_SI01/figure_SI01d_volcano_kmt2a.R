...
labelColor <- "#555555"
segmentColor <- "#AAAAAA"
xLabel <- expression(paste(log[2], " fold-change KMT2A rearranged vs. Rest BCP-ALL"))
yLabel <- expression(paste(-log[10]~italic(p), "-value"))
densityColor <- "Red"

# labeling
genesToLabel <- c("FLT3", "KLRK1", "HOXA9", "RUNX2", "TP53", "MEIS1", "CD9", "DNTT", "MDM2", "FLT1", "BLNK", "DUSP1", "MAPK13", "CD200", "LRRC14B", "SMAD1", "CDKN2A", "CDKN1B",
                  "PROM1","TP53", "RYBP", "IGF1R", "CDKN2C", "PBX3")

# read DEqMS result
file <- file.path("output", useVersion, "de", "KMT2A_vs_BCP-ALL", "KMT2A-BCPALL_DEqMS_table.txt")
