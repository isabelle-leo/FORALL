...
labelColor <- "#555555"
segmentColor <- "#AAAAAA"
xLabel <- expression(paste(log[2], " fold-change TCF3-PBX1 vs. Rest BCP-ALL"))
yLabel <- expression(paste(-log[10]~italic(p), "-value"))
densityColor <- "Red"

# labeling
genesToLabel <- c("ANKS1B", "RORB", "PHACTR3", "FAT1", "WNT16", "MERTK", "NLGN1", "ROR1",
                  "PBX1", "IGHM", "IGHD", "HMGA2")

# read DEqMS result
file <- file.path("output", useVersion, "de", "TCF3-PBX1_vs_BCP-ALL", "TCF3.PBX1-BCPALL_DEqMS_table.txt")
