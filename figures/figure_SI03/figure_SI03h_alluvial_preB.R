...
allmeta <- read.csv(file = file.path("meta", "all_meta_reclassified.csv"), row.names = "Cell.Line.Name") %>% #read in file created during Seurat Vln plot classification script
...
sankeymeta_2 <- allmeta %>%
  ...
  filter(Type == "preB") %>%
  dplyr::select(AssignedStages, Subtype_Paper, hierarCluster_pearson_ward.D2_B)
...
stage_to_subtype_and_cluster <- sankeymeta_2 %>%
  group_by(AssignedStages, Subtype_Paper, hierarCluster_pearson_ward.D2_B) %>%
  summarise(connection_count = n())

color_scheme <- getColorScheme()

color_scheme$AssignedStages <- c("pre-B other" = "#fb8072", "pre-pro-B" = "#80b1d3",
...
p1 <- ggplot(as.data.frame(stage_to_subtype_and_cluster),
             aes(y = connection_count,
                 axis1 = Subtype_Paper, axis2 = AssignedStages, axis3 = hierarCluster_pearson_ward.D2_B)) +
