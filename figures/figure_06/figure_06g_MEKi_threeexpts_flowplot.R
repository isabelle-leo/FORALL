...
# remove cell lines with "other" subtype
...
#MAIN PLOT: DMSO NORMALIZE, BRYO VERSUS TREATED
subset_raw_counts_plus_main <- subset_raw_counts_plus[subset_raw_counts_plus$SubGroup != "1uM Tramatenib",]
subset_raw_counts_plus_main <- subset_raw_counts_plus_main[subset_raw_counts_plus_main$SubGroup != "1uM UO126",]
subset_raw_counts_plus_main <- subset_raw_counts_plus_main[subset_raw_counts_plus_main$SubGroup != "1uM Selumetinib",]
subset_raw_counts_plus_main <- subset_raw_counts_plus_main[subset_raw_counts_plus_main$SubGroup != "1uM ERK11e",]
subset_raw_counts_plus_main <- subset_raw_counts_plus_main[subset_raw_counts_plus_main$SubGroup != "25nM PMA",]
subset_raw_counts_plus_main <- subset_raw_counts_plus_main[subset_raw_counts_plus_main$SubGroup != "25nM PMA, 1uM UO126",]
subset_raw_counts_plus_main <- subset_raw_counts_plus_main[subset_raw_counts_plus_main$SubGroup != "25nM PMA, 1uM Tramatenib",]
subset_raw_counts_plus_main <- subset_raw_counts_plus_main[subset_raw_counts_plus_main$SubGroup != "25nM PMA, 1uM Selumetinib",]
subset_raw_counts_plus_main <- subset_raw_counts_plus_main[subset_raw_counts_plus_main$SubGroup != "25nM PMA, 1uM Erk11e",]

experiments <- unique(subset_raw_counts_plus_main$ExperimentDate)
#make empty data frame for filling
normalized.to.dmso <- subset_raw_counts_plus_main
...
for (i in 1:length(experiments)){
  ...
  one_experiment_subset <- subset_raw_counts_plus_main[subset_raw_counts_plus_main$ExperimentDate == experiments[i],]
...
p <- ggplot(data = normalized.to.dmso, aes(x = SubGroup, y = NormalizedDMSO), show.legend = F) + 
  ...
  xlab("") + 
  geom_jitter(size = 1.5, stroke = 0, alpha = 0.8, aes(x = SubGroup, y = NormalizedDMSO, colour = CellType)) + 
...
library("ggpubr")
compare_means(Count ~ DrugType, data = subset_raw_counts_plus, method = "t.test", paired = FALSE) #for raw counts
my_comparisons <- list(c("100nM Bryostatin-1", "100nM Bryostatin-1, 1uM UO126"), 
                       c("100nM Bryostatin-1", "100nM Bryostatin-1, 1uM Tramatenib"),
                       c("100nM Bryostatin-1", "100nM Bryostatin-1, 1uM Selumetinib"),
                       c("100nM Bryostatin-1", "100nM Bryostatin-1, 1uM Erk11e"),
                       c("100nM Bryostatin-1", "DMSO"))

p <- p + stat_compare_means(comparisons = my_comparisons, method = "t.test", paired = FALSE) #for normalized ggplot data

p

# export to svg
ggsave(
  "flow_threeexpts_meki.svg",
  plot = p,
  device = NULL,
  path = outputFolder,
  scale = 1,
