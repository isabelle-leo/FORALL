...
# Run this to plot T cells only
fileName <- "T_alluvial_plot.svg"
sankeymeta <- allmeta %>%
  dplyr::filter(Type_Paper == "T-ALL") %>%
  dplyr::select(Subtype_Paper, proteinCluster = hierarCluster_pearson_ward.D2_T, rnaCluster = RNA_T)
