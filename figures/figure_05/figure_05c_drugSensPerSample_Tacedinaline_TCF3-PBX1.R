...
library(svglite)
ggsave(
  "figure_05_drugSensPerSample_Tacedinaline_TCF3-PBX1_rev.svg",
  plot = p,
  device = NULL,
  path = outputFolder,
  scale = 1,
...
# export to png
ggsave(
  "Supp_Figure_XX_drugSensPerSample_Idasanutlin.png",
  plot = p,
  device = NULL,
  path = outputFolder,
  scale = 1,
...
# without subtype coloring
# df %>%
#   ggplot(aes(x = Cell.Line.R, y = sdss)) +
#   geom_hline(yintercept = 0,
#              color = "gray50") +
#   geom_segment(aes(xend = Cell.Line.R,
#                    y = 0,
...
#              size = 10,
#              color = colorScheme$red,
#              alpha = 0.7) + # scatter and lollopop plot
#   geom_point(size = 5,
#              color = colorScheme$blue) + # scatter and lollopop plot
#   # geom_bar(stat = "identity") + # bar plot
#   xlab("Cell line") +
#   ylab("sDSS") +
#   ggtitle(df$DRUG.NAME %>% unique() %>% paste(collapse = ", ")) +
