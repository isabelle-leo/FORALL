...
#elbow plot - Seurat function
#pdf(file="elbow_plot_pcs.pdf", width=7, height=6)
ElbowPlot(seurat.object) + theme_minimal()
#dev.off()

#jack straw plot - Seurat function
seurat.object <- JackStraw(seurat.object, num.replicate = 100, dims = 55) #replicate number chosen to speed up analysis
...
umap.output <- uwot::umap(data.matrix(t(sparse.matrix.filtered)),
                          ...
                          pca = 27, init = "normlaplacian", #SET NUMBER OF PCS IN PCA ARGUMENT - from jack straw
...
df.plot <- as.data.frame(seurat.object@reductions[["umap"]]@cell.embeddings)
df.plot$join <- rownames(df.plot)
meta <- as.data.frame(seurat.object@meta.data)
meta$join <- rownames(meta)
df.plot <- left_join(x = df.plot, y = meta, by = "join")

plot <- ggplot(data = df.plot, aes(x = UMAP_1, y = UMAP_2, colour=Specific.annotation, fill = Specific.annotation)) +
...
drug.from.text <- c("Mubritinib","Tacedinaline","BAY_87-2243", "NMS-873", "Romidepsin", "Panobinostat", 
                    "Vorinostat", "Belinostat", "Imatinib", "Dasatinib", "Nilotinib", "Sorafenib", "Sitravatinib", "Cabozantinib", "AMG319", "Palomid_529", "Duvelisib", "AZD_8186",
                    "NVP_BGT226")
...
#pdf(file=file.path(outputFolder, "point_plot_umap_drugs_labels_limitedv2.pdf"), width=6, height=6)
